...
word2vec = gensim.models.Word2Vec.load('./m_clothes_model60000_50_4')
...
vocab_to_idx, idx_to_vocab, idx_to_count = scan_vocabulary(
    comments, min_count=50, verbose=True)
...
f = open("m_brand_name_ner_50_4.txt", 'w')

#아 기억났다 _는 버리는 변수명으로 잘 쓰인댔어
#seed_words 타입은 set이야 집합!!
p_seed_list = ['나이키/Noun', '데상트/Noun', '파타고니아/Noun', '르까프/Noun', '디스커버리/Noun', '휠라/Noun', '퓨마/Noun']
...
n_seed_list = ['사이즈/Noun', '자켓/Noun', '바람막이/Noun', '한정판/Noun', '빨검/Noun', '숏/Noun', '수세미/Noun',
            '수면/Noun', '등산/Noun', '눕다/Verb', '검다/Adjective', '축구화/Noun', '이동수/Noun', '바막/Noun', '골키퍼/Noun', '영원/Noun', '브라보/Noun', '후반/Noun', '카모/Noun']
...
real_n_seed_list = ['검은색/Noun', '베이지/Noun']

real_nseed_words = {word for word, _ in word2vec.wv.most_similar(real_n_seed_list[0], topn=10) if word.split('/')[1] == 'Noun'}
for nword in real_n_seed_list[1:]:
    real_nseed_words.update({word for word, _ in word2vec.wv.most_similar(nword, topn=10) if word.split('/')[1] == 'Noun'})
...
for word, prob in sorted(pred_pos.items(), key=lambda x:-x[1])[:600]:
