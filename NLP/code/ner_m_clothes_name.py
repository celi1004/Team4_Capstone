...
word2vec = gensim.models.Word2Vec.load('./m_clothes_model60000_120_4')
...
comments += read_data('./data/mbottom.txt')
comments += read_data('./data/mtop2.txt')
comments += read_data('./data/mbottom2.txt')
comments += read_data('./data/mtop3.txt')
comments += read_data('./data/mbottom3.txt')
...
vocab_to_idx, idx_to_vocab, idx_to_count = scan_vocabulary(
    comments, min_count=120, verbose=True)
...
f = open("m_clothes_name_ner_120_4.txt", 'w')

#아 기억났다 _는 버리는 변수명으로 잘 쓰인댔어
#seed_words 타입은 set이야 집합!!
p_seed_list = ['스트라이프/Noun','가죽/Noun','코트/Noun','후드/Noun','티/Noun','모직/Noun','스판/Noun',
            '워싱/Noun', '중청/Noun', '가디건/Noun', '면/Noun', '팬츠/Noun',
            '집업/Noun', '기모/Noun']

pseed_words = {word for word, _ in word2vec.wv.most_similar(p_seed_list[0], topn=10) if word.split('/')[1] == 'Noun'}
for pword in p_seed_list[1:]:
    pseed_words.update({word for word, _ in word2vec.wv.most_similar(pword, topn=10) if word.split('/')[1] == 'Noun'})

pseed_words = pseed_words | set(p_seed_list)
print(len(pseed_words)) 

n_seed_list = ['실리콘/Noun', '포토북/Noun', '럭셔리/Noun', '어깨/Noun', '풋/Noun', '라코스테/Noun', '유니클로/Noun', '인디안/Noun',
            '숏/Noun', '네이비/Noun', '블랙/Noun', '내셔널/Noun', '깨끗/Noun', '어린이집/Noun', '여름/Noun', '발렌시아/Noun', '바나나/Noun', '명동/Noun']
...
real_n_seed_list = ['네이비/Noun', '블랙/Noun', '발렌시아/Noun']
...
for word, prob in sorted(pred_pos.items(), key=lambda x:-x[1])[:500]:
