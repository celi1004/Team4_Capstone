...
vocab_to_idx, idx_to_vocab, idx_to_count = scan_vocabulary(
    comments, min_count=60, verbose=True)
...
f = open("clothes_color_ner_60_4.txt", 'w')

#아 기억났다 _는 버리는 변수명으로 잘 쓰인댔어
#seed_words 타입은 set이야 집합!!
p_seed_list = ['그레이/Noun', '검/Noun', '오렌지/Noun', '화이트/Noun', '와인/Noun', '아이보리/Noun']

pseed_words = {word for word, _ in word2vec.wv.most_similar(p_seed_list[0], topn=17) if word.split('/')[1] == 'Noun'}
for pword in p_seed_list[1:]:
    pseed_words.update({word for word, _ in word2vec.wv.most_similar(pword, topn=17) if word.split('/')[1] == 'Noun'})

pseed_words = pseed_words | set(p_seed_list)
print(len(pseed_words)) 

n_seed_list = ['나이키/Noun', '프리미엄/Noun', '에센셜/Noun', '사이즈/Noun', '르샵/Noun', '파이어버드/Noun', '테니스화/Noun', '프린트/Noun'
            ,'삼선/Noun', '벽돌/Noun', '모카/Noun', '도트/Noun', '투맨/Noun', '축구공/Noun', '바리스타/Noun', '시난주/Noun', '팀버/Noun', '햄/Noun', '멀티/Noun']

nseed_words = {word for word, _ in word2vec.wv.most_similar(n_seed_list[0], topn=9) if word.split('/')[1] == 'Noun'}
for nword in n_seed_list[1:]:
    nseed_words.update({word for word, _ in word2vec.wv.most_similar(nword, topn=9) if word.split('/')[1] == 'Noun'})
