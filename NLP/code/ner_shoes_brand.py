...
word2vec = gensim.models.Word2Vec.load('./shoes_model90000_90_6')
...
comments = read_data('./data/mshoes.txt')
comments += read_data('./data/mshoes2.txt')
comments += read_data('./data/mshoes3.txt')
comments += read_data('./data/mtop.txt')
comments += read_data('./data/ftop.txt')
comments += read_data('./data/fshoes.txt')
comments += read_data('./data/fshoes2.txt')
comments += read_data('./data/mtop2.txt')
...
vocab_to_idx, idx_to_vocab, idx_to_count = scan_vocabulary(
    comments, min_count=90, verbose=True)
...
f = open("shoes_brand_ner_90_4.txt", 'w')

#아 기억났다 _는 버리는 변수명으로 잘 쓰인댔어
#seed_words 타입은 set이야 집합!!
p_seed_list = ['돌체앤가바나/Noun', '데상트/Noun', '파타고니아/Noun', '르까프/Noun', '디스커버리/Noun', '휠라/Noun', '퓨마/Noun', '닥마/Noun']

pseed_words = {word for word, _ in word2vec.wv.most_similar(p_seed_list[0], topn=40) if word.split('/')[1] == 'Noun'}
for pword in p_seed_list[1:]:
    pseed_words.update({word for word, _ in word2vec.wv.most_similar(pword, topn=40) if word.split('/')[1] == 'Noun'})

pseed_words = pseed_words | set(p_seed_list)
print(len(pseed_words)) 

n_seed_list = ['사이즈/Noun', '런닝/Noun', '런닝화/Noun', '형광/Noun', '한정판/Noun', '워커/Noun', '향수/Noun', '불꽃/Noun', '갈색/Noun'
            , '등산/Noun', '테니스/Noun', '마라톤/Noun', '슈퍼스타/Noun', '빅/Noun']

nseed_words = {word for word, _ in word2vec.wv.most_similar(n_seed_list[0], topn=40) if word.split('/')[1] == 'Noun'}
for nword in n_seed_list[1:]:
    nseed_words.update({word for word, _ in word2vec.wv.most_similar(nword, topn=40) if word.split('/')[1] == 'Noun'})
